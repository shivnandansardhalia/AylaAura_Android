...
public class CloudDatapointTestCase extends TestCase {
    public CloudDatapointTestCase() {
        super("Cloud Datapoint Test");
    }
    ...
    public void run(TestSuite suite) {
        ...
        blueLED.createDatapointCloud(1, null, future, future);
        AylaDatapoint dp;
        try {
        ...
        greenLED.createDatapointCloud(1, null, future, future);
        try {
            dp = future.get(3000, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            fail();
        ...
        if (!String.valueOf(dp.getValue()).equals( "1")) {
            fail();
            suite.logMessage(LogEntry.LogType.Error, "Returned datapoint value is " + dp.getValue());
            return;
        }
        ...
        future = RequestFuture.newFuture();
        blueLED.createDatapointCloud(0, null, future, future);
        try {
            dp = future.get(3000, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            fail();
        ...
        future = RequestFuture.newFuture();
        greenLED.createDatapointCloud(0, null, future, future);
        try {
            dp = future.get(3000, TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            fail();
